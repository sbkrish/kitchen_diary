...
<nav class="navbar navbar-default navbar-fixed-top">
  <div class="container">
     <a class="navbar-brand" style="text-align: center;"><font color="#134756"><b>Kitchen Diary</b></font></a> 
...
$sql=mysqli_query($con, "SELECT Item,Quantity,Price,Buyer FROM `daily_purchase` WHERE `Date`='$date' AND Price>0 AND Quantity>0");
if(!($sql))
	{
	...
	else
	{
			...
			if(mysqli_num_rows($sql)==0)
			{
				...
				{*/
					?>
					<div class="container">
<h4><?php $received_date.""."'s Purchase" ?></h4>
<div class="panel">  
...
<?php
				...
				$total=mysqli_query($con, "SELECT SUM(Price) AS `total_amount` FROM `daily_purchase` WHERE `Date`='$date' AND `Quantity`>0 AND `Price`>0");
