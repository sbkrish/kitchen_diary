...
$which_month=$_SESSION["which_month"];
...
$sql=mysqli_query($con, "SELECT `Date`, `Buyer`, SUM(`Price`) AS `Price` FROM `daily_purchase` WHERE Month='$month' AND Year='$year'  GROUP BY `Date`, `Buyer` ORDER BY `Id`");
...
if(!($sql))
	...
	else
	{
			...
			else
			{
				...
				<p>Month: <b><?php echo "$month".'-'."$year";?></b></p>
	...
	<?php
				...
				{
								echo "<tr><td class='third'>{$jack['Date']}</td><td class='first'>{$jack['Buyer']}</td><td class='second'>{$jack['Price']}</td></tr>\n";
				}
...
<p class="detailed">For detailed expenses <a href="dateselectviewdiary.php">click here</a></p>
</div>
<div class="back">
